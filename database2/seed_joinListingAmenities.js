  ...
  function writeJoin() {
    while (currlistingid < whilemax) {
      ...
      for (let i = 0; i < cat1and2.length; i++) {
        data = generateJoinListingAmenity(joinid, currlistingid, cat1and2[i]);
        joinid++;
        glistingcount++;
        alldata += data;
      };
      ...
      let maxmixedcategories = Math.floor(Math.random() * 3) + 1; // random number from 1 to 3
